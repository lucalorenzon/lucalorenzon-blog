...
| 2026-08-23 | Completata l'implementazione di EP-001-UC-001-S001: `ContentSource`/`FetchError` (`src/domain/ports.rs`), `FilesystemContentSource` (parsing YAML via `serde_yaml`, scelta esplicita dell'utente pur essendo il crate deprecato dal 2024) e `InMemoryContentSource`. 19 test verdi (14 dominio+fake, 5 integrazione filesystem su file temporanei reali). Tutti gli AC/AT della story, incluso AC-6, ora coperti — story di fatto completa, resta aperta la sola integrazione con la pipeline CI (S004) |
