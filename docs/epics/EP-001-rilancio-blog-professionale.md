...
|---|---|---|
| AC-1 | Il sito compila e genera un output statico deployabile senza processo server sempre attivo | Esecuzione della build e verifica dell'assenza di un processo server in produzione — **build sbloccata** (2026-08-20); **output ora SSG** (2026-09-01, EP-001-UC-001-S003: composition root riscritto su `StaticRouteGenerator`, verificato con build reale), server always-on ritirato in produzione |
...
| [ADR-004](../adr/ADR-004-ssg-islands-replaces-ssr.md) | SSG + islands al posto di SSR | 2026-08-24 | Accepted (implementata in EP-001-UC-001-S003, verificata con build reale 2026-09-01) |
...
| 2026-08-23 | Ricerca di mercato su CMS git-based confermata: ADR-002 resta valida (Sveltia CMS notato come opzione UI futura non prioritaria). Creata ADR-003 (topologia repo e hosting GitHub Pages): 2 repo, `lucalorenzon-blog` da rendere pubblico invece di GitHub Pro, repo contenuti dedicato può restare privato |
| 2026-08-23 | Eseguita ADR-003: scansionata la storia di `lucalorenzon-blog` per credenziali (nessuna trovata); riscritti 4 commit pre-2026 che esponevano un'email di lavoro, force-push su origin. Repo reso pubblico e sottoposto a hardening (branch protection su main senza richiedere PR, secret scanning + push protection, Dependabot alerts/security/version updates, private vulnerability reporting). Creato `lucalorenzon-blog-content` (privato), scaffolded con README e articolo di esempio — ADR-002 e ADR-003 aggiornate di conseguenza |
| 2026-08-23 | Completata l'implementazione di EP-001-UC-001-S001: `ContentSource`/`FetchError` (`src/domain/ports.rs`), `FilesystemContentSource` (parsing YAML via `yaml_serde`, mantenuto da The YAML Organization) e `InMemoryContentSource`. 19 test verdi (14 dominio+fake, 5 integrazione filesystem su file temporanei reali). Tutti gli AC/AT della story, incluso AC-6, ora coperti — story di fatto completa, resta aperta la sola integrazione con la pipeline CI (S004) |
| 2026-08-24 | Completata EP-001-UC-001-S002 (unicità slug): `ContentSource::exists`, dominio `SlugUniquenessError`/`ensure_slug_is_unique`. Size **S** |
| 2026-09-01 | Completata (chiusa formalmente a posteriori) EP-001-UC-001-S003 (SSG: ARTICLE-PAGE/LISTING-PAGE/HOME-PAGE, ADR-004 implementata). Verifica end-to-end con build reale ha trovato e corretto 3 bug reali (path fallback immagine, path immagine propria dell'articolo mai copiata nel sito generato). Size **XL** — assegnata retroattivamente, eccezione nota: story pianificata/condotta sotto un sistema di regole LLM precedente, servita anche da banco di prova per le modifiche al nuovo sistema; non passata dal gate odierno "XL/XXL → torna a `/sw-story-split`". **Emerso un candidato a thin epic non ancora creato**: storage immagini articolo proprio (oggi fix minimo — immagini copiate da `images_dir` del repo contenuti in `site-root/images` a build time — location, collisioni e porta `AssetSource` dedicata restano da decidere in una story/epic separata) |
| 2026-09-01 | Creato [EP-002](EP-002-contenuto-visivo-scalabile-articoli.md) — contenuto visivo scalabile per gli articoli: storage immagini disaccoppiato da git (`AssetSource`, limiti spazio GitHub free tier) + adattamento responsive per breakpoint (copertina e immagini nel corpo del testo). Assorbe il residuo #7 dello Stressor Analysis di S003 |
