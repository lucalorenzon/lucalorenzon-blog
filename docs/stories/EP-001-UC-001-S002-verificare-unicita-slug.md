...
| 2026-08-24 | `/sw-practices`: implementato `ContentSource::exists` su entrambi gli adapter (`FilesystemContentSource`: `std::fs::metadata`, nessuna lettura del contenuto; `InMemoryContentSource`: `HashMap::contains_key`), `SlugUniquenessError` e `ensure_slug_is_unique` in `src/domain/slug_uniqueness.rs`. Nessuna nuova dipendenza (`thiserror` già presente). `cargo build`/`cargo test` (19 test preesistenti, tutti verdi) e `cargo fmt --check` puliti; `cargo clippy -D warnings` non introduce nulla di nuovo sui file toccati (i 6 errori esistenti sono preesistenti, fuori scope di questa story) | Riuso del pattern I/O già stabilito in S001 (`ErrorKind::NotFound` → esito applicativo, non errore); nessun bootstrap da toccare, nessun composition root ancora cablato (resta S004) | — |
